<div>
    <x-partials.title :title="$category->name" />

    <div class="container mt-36">
        <div class="grid grid-cols-4 gap-5">
            @foreach($category->projects as $project)
                <a href="{{ route('projects.project', $project->id) }}" class="inline-block border border-gray-200 rounded-lg p-3">
                    <img src="/storage/{{ $project->cover }}" class="rounded-lg w-full" alt="">
                    <h1 class="text-3xl font-bold mt-3">{{ $project->title }}</h1>
                    <p class="text-gray-500 mt-3 mb-5">{!! Str::limit($project->description, 150)  !!}</p>
                    <div>
